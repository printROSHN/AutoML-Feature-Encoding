...
from utils import cat_cols_info, save_dict_to_file
...
def execute_experiment(dataset_name, encoders_list, validation_type, file_name_apex, experiment_description):
    dataset_pth = f"C:/CODE/JIO (JPL)/CategoricalEncodingBenchmark-master/src/data/{dataset_name}.csv"
    results = {}

    # training params
    N_SPLITS = 5
    model_validation = StratifiedKFold(
        n_splits=N_SPLITS, shuffle=True, random_state=42)
    encoder_validation = StratifiedKFold(
        n_splits=N_SPLITS, shuffle=True, random_state=2019)

    # load processed dataset
    data = pd.read_csv(dataset_pth)
    target = data['target']
    #data['num_19'] = data['num_19'].astype(float)
    ...
    X_train, X_test = X_train.reset_index(
        drop=False), X_test.reset_index(drop=False)
    ...
    # print("Y_test", y_test)
    # print("--------------------------------------------")
    # print("mean_target_train", np.mean(y_train))
    # print("--------------------------------------------")
    # print("mean_target_test", np.mean(y_test))
    # print("--------------------------------------------")
    # print("num_cat_cols",len(cat_cols))
    # print("--------------------------------------------")
    # print("cat_cols_info",cat_cols_info(X_train, X_test, cat_cols))
    # print("--------------------------------------------")
    ...
    for encoders_tuple in encoders_list:
        ...
        #                   encoders_names=encoders_tuple, cat_cols=cat_cols)


        label = LabelEncoder()
        for column_name in cat_cols:
            X_train[column_name] = label.fit_transform(X_train[column_name])
        print(X_train)
        X_train.to_csv('train_data.csv', index=False)

        # train_score, val_score, avg_num_trees = SumEncoder.fit(X_train, y_train)
    ...
    # save_dict_to_file(
    #     dic=results[dataset_name], path=f"./results/{file_name_apex}{dataset_name}.txt", save_raw=False)
    # save_dict_to_file(
    #     dic=results[dataset_name], path=f"./results/{file_name_apex}{dataset_name}_r.txt", save_raw=True)
...
if __name__ == "__main__":
    ...
    for dataset_name in dataset_list:
        ...
        execute_experiment(dataset_name, encoders_list,
                           validation_type, file_name_apex, experiment_description)
        ...
        experiment_description = f"Check single encoder, {validation_type} validation"
        execute_experiment(dataset_name, encoders_list_double,
                           validation_type, file_name_apex, experiment_description)
